...
if options.PA:
    ...
    for MAFc,v in sorted(Counts.items()):
        for Pop,v1 in sorted(v.items()):
            ...
            for chrom,v2 in sorted(v1.items()):
                if v2["SS"]==0:
                    out.write(MAF[:1]+"."+MAF[1:]+"\t"+MAC+"\t"+str(MAFc)+"\t"+header[Pop]+"\t"+chrom+"\t"+str(v2["NS"])+"\t"+str(v2["SS"])+"\tNA\n")
                else:
                    out.write(MAF[:1]+"."+MAF[1:]+"\t"+MAC+"\t"+str(MAFc)+"\t"+header[Pop]+"\t"+chrom+"\t"+str(v2["NS"])+"\t"+str(v2["SS"])+"\t"+str(v2["NS"]/v2["SS"])+"\n")
                ns+=v2["NS"]
                ss+=v2["SS"]
            if ss==0:
                out.write(MAF[:1]+"."+MAF[1:]+"\t"+MAC+"\t"+str(MAFc)+"\t"+header[Pop]+"\tgenomewide\t"+str(ns)+"\t"+str(ss)+"\tNA\n")
            else:
                out.write(MAF[:1]+"."+MAF[1:]+"\t"+MAC+"\t"+str(MAFc)+"\t"+header[Pop]+"\tgenomewide\t"+str(ns)+"\t"+str(ss)+"\t"+str(ns/ss)+"\n")
else:
    out=gzip.open(options.OUT,"wt")
    ...
    for MAF,v in sorted(Counts.items()):
        for Pop,v1 in sorted(v.items()):
            ...
            for chrom,v2 in sorted(v1.items()):
                if v2["SS"]==0:
                    out.write(str(MAF)+"\t"+header[Pop]+"\t"+chrom+"\t"+str(v2["NS"])+"\t"+str(v2["SS"])+"\tNA\n")
                else:
                    out.write(str(MAF)+"\t"+header[Pop]+"\t"+chrom+"\t"+str(v2["NS"])+"\t"+str(v2["SS"])+"\t"+str(v2["NS"]/v2["SS"])+"\n")
                ns+=v2["NS"]
                ss+=v2["SS"]
            if ss==0:
                out.write(str(MAF)+"\t"+header[Pop]+"\tgenomewide\t"+str(ns)+"\t"+str(ss)+"\tNA\n")
            else:
                out.write(str(MAF)+"\t"+header[Pop]+"\tgenomewide\t"+str(ns)+"\t"+str(ss)+"\t"+str(ns/ss)+"\n")
