...
#Parse the allele phylogenetic concrodance data
 unlist_vect = lapply( priv.dt[,7] , unlist )
...
o_dcast %<>% mutate(caller = caller[1], Total_obs = `0`+`1`) %>% mutate(Probality_obs = `1`/Total_obs)
...
## Save
save(expected_o, o_dcast, file="./PoolSNP.Probability_CC.Rdata")


### Plot
load("./PoolSNP.Probability_CC.Rdata")
...
ggsave("prob_graph.pdf",prob_graph, width = 4, height = 5)
