...
### caller[2] is SANPE -- unless overwriten by user
  priv.dt <- fread("/scratch/yey2sn/DEST_mutation/geo_endemic.SNAPE.MAF001.goodSamps.delim")
...
#Parse the allele frequency data 
 ...
 # -> tmp 
 #save(tmp, file="./snape.tmp.Rdata")
 #load("./snape.tmp.Rdata")
 
 o[,c("nPop","AF","Freq")] = sapply(o[,c("nPop","AF","Freq")], as.numeric)
 o %<>% mutate(caller = caller[2])
 save(o, file="./SNAPE.0.001.mutation.Rdata")
 
 
 ### Plot
 load("./SNAPE.0.001.mutation.Rdata")
 
 o$AF = as.numeric(o$AF)
 ...
 o %>% 
   ...
   mutate(Perc = prop.test(N,Tot)$estimate) %>%
   mutate(p.value = prop.test(N,Tot)$p.value) %>%
   mutate(type = "common", caller = "SNAPE") -> 
   total_count_common
 
 rbind(total_count_rare,total_count_common) -> joint_o_SNAPE
 
 save(joint_o_SNAPE, file="./SNAPE_mutation_imput.Rdata")
